...
console.log(
  chalk.bold.yellow("I have added only 3 bank accounts, 1001, 1002, 1003")
);
...
class BankAccount implements BankAccount {
  ...
  deposit(amount: number): void {
    ...
    console.log(
      chalk.bold.green(
        `Deposit of $${amount} successful, New balance is: $${this.balance}`
      )
    );
  }
...
async function service() {
  do {
    ...
    if (customer) {
      console.log(
        chalk.bold.yellow(
          `Welcome, ${chalk.bold.red(customer.firstName)} ${chalk.bold.red(
            customer.lastName
          )}!\n`
        )
      );
